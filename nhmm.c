...
int main() {
    ...
    */
    ...
    char ciphertext;
    FILE * fptr;
    fptr = fopen("/Users/sahilmotadoo/Documents/cs286/assign2/brown.txt", "r");
    if (fptr) {
    ...
    while (currentModel < model) {
        iters = 0;
        logProb = 0;
        printf("\nHMM Random Restarts Substitution Cipher...\n\n");
        /* 
    ...
    */
        ...
        if (iters < maxIterations || logProb > oldLogProb) {
            ...
            if (logProb > oldModelProb) {
                oldModelProb = logProb;
                for (int i = 0; i < N; i++) {
                    for (int j = 0; j < N; j++)
                        oldA[i][j] = A[i][j];
                }
